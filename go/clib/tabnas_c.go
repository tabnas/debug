...
// Package main builds the C-ABI shared library: libtabnasdebug.
//
// tabnas-clib-template: v4 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasdebug.so ./clib
//
// debug TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a debug parser and returns a handle to it.
...
// tabnas_parse checks one input against the debug grammar.
//
//export tabnas_parse
